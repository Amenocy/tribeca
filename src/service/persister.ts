...
export interface ILoadAll<T> extends IPersist<T> {
    loadAll(limit?: number, query?: Record<string, any>): Promise<T[]>;
}
...
export class Persister<T extends Persistable> implements ILoadAll<T> {
    ...
    public loadAll = (limit?: number, query?: any): Promise<T[]> => {
        const selector: Record<string, any> = { exchange: this._exchange, pair: this._pair };
        _.assign(selector, query);
        return this.loadInternal(selector, limit);
    };

    private loadInternal = async (selector: Record<string, any>, limit?: number): Promise<T[]> => {
        ...
        this._log.info("load docs completed", {
            selector: selector,
            limit: limit,
